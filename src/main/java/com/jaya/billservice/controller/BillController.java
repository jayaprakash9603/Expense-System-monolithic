package com.jaya.billservice.controller;


import com.jaya.billservice.dto.BillRequestDTO;
import com.jaya.billservice.dto.BillResponseDTO;
import com.jaya.billservice.dto.ProgressStatus;
import com.jaya.billservice.kafka.service.BillNotificationService;
import com.jaya.billservice.mapper.BillMapper;
import com.jaya.billservice.models.Bill;
import com.jaya.billservice.service.BillService;
import com.jaya.billservice.service.ExcelExportService;
import com.jaya.billservice.util.BulkProgressTracker;
import com.jaya.billservice.util.ServiceHelper;
...
public class BillController {
    ...
    private User getTargetUserWithReadAccess(Integer targetId, User reqUser) throws Exception {
        if (targetId == null)
            return reqUser;
        User targetUser = helper.validateUser(targetId);
        if (targetUser == null)
    ...
    public ResponseEntity<BillResponseDTO> createBill(@RequestBody BillRequestDTO billDto,
        ...
        Bill bill = BillMapper.toEntity(billDto, targetUser.getId());
        ...
        BillResponseDTO resp = BillMapper.toDto(createdBill);
    ...
    public ResponseEntity<List<BillResponseDTO>> addMultipleBills(@RequestHeader("Authorization") String jwt,
        ...
        User reqUser = userService.getUserProfile(jwt);
        User targetUser = getTargetUserWithPermissionCheck(targetId, reqUser);
        List<Bill> toCreate = bills.stream().map(dto -> BillMapper.toEntity(dto, targetUser.getId()))
                .toList();
        List<Bill> saved = billService.addMultipleBills(toCreate, targetUser.getId());
        List<BillResponseDTO> resp = saved.stream().map(BillMapper::toDto).toList();
        ...
        taskExecutor.execute(() -> {
            try {
                List<Bill> toCreate = bills == null ? java.util.Collections.emptyList()
                        : bills.stream().map(dto -> BillMapper.toEntity(dto, targetUser.getId()))
                                .toList();
    ...
    public ResponseEntity<Bill> getBillById(@PathVariable Integer id, @RequestHeader("Authorization") String jwt,
        ...
        try {
            User reqUser = userService.getUserProfile(jwt);
            User targetUser = getTargetUserWithReadAccess(targetId, reqUser); // read access only
    ...
    public ResponseEntity<BillResponseDTO> updateBill(@PathVariable Integer id, @RequestBody BillRequestDTO billDto,
        ...
        BillResponseDTO resp = BillMapper.toDto(updatedBill);
        return ResponseEntity.ok(resp);
    }
    ...
    public ResponseEntity<?> getAllBills(
        ...
        try {
            User reqUser = userService.getUserProfile(jwt);
            User targetUser = getTargetUserWithReadAccess(targetId, reqUser); // listing -> read access
            List<Bill> bills;
            ...
            com.jaya.billservice.response.Error error = new com.jaya.billservice.response.Error();
    ...
    public ResponseEntity<?> importAndSaveBillsFromExcel(
        ...
        try {
            ...
            List<Bill> billsToSave = importedBills.stream()
                    .map(dto ->BillMapper.toEntity(dto, targetUser.getId()))
                    .toList();
