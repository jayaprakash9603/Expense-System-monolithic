package com.jaya.billservice.service;

import com.jaya.billservice.models.Bill;
import com.jaya.billservice.repository.BillRepository;
...
public class BillServiceImpl implements BillService {
    ...
    public Bill updateBill(Bill bill, Integer userId) throws Exception {
        try {
            ...
            expense.setDate(LocalDate.parse(bill.getDate().toString()));
            ...
            Expense savedExpense = expenseService.updateExpenseWithBillService(existingBill.getExpenseId(), expense,
                    user.getId());
    ...
    public List<ExpenseDTO> getAllExpensesForBill(Integer userId, LocalDate startDate, LocalDate endDate)
        ...
        return bills.stream()
                .filter(bill -> bill.getExpenseId() != null)
                .map(bill -> expenseService.getExpenseById(bill.getExpenseId(), userId)) // Expense
                .filter(Objects::nonNull)
                .map(expense -> helper.mapExpenseToDto(expense)) // or expenseMapper.toDto(expense)
                .collect(Collectors.toList());
    }
    ...
    public List<Bill> addMultipleBills(List<Bill> bills, Integer userId) throws Exception {
        ...
        int count = 0;
        List<Bill> savedBills = new ArrayList<>(bills.size());
