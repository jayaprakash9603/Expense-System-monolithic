...
import com.jaya.auditservice.models.AuditEvent;
...
import com.jaya.expenseservice.async.AsyncExpensePostProcessor;
...
import com.jaya.expenseservice.exceptions.ResourceNotFoundException;
import com.jaya.expenseservice.exceptions.UserException;
import com.jaya.expenseservice.kafka.AuditEventProducer;
import com.jaya.expenseservice.kafka.BudgetExpenseKafkaProducerService;
import com.jaya.expenseservice.kafka.CategoryExpenseKafkaProducerService;
import com.jaya.expenseservice.kafka.PaymentMethodKafkaProducerService;
...
import com.jaya.expenseservice.repository.ExpenseReportRepository;
import com.jaya.expenseservice.repository.ExpenseRepository;
import com.jaya.expenseservice.service.KafkaProducerService;
...
import com.jaya.expenseservice.util.JsonConverter;
import com.jaya.expenseservice.util.ServiceHelper;
import com.jaya.paymentservice.dto.PaymentMethodEvent;
...
public class ExpenseCoreServiceImpl implements ExpenseCoreService {
    ...
    public void deleteExpense(Integer id, Integer userId) throws Exception {
        ...
        if (expense.getExpense() != null && expense.getExpense().getPaymentMethod() != null) {
            ...
            try {
                PaymentMethod paymentMethod = paymentMethodService.getByNameAndTypeOrCreate( paymentMethodName,
    ...
    private void publishBulkSummaryAudit(String actionType, List<Expense> expenses, User user, String details) {
        if (expenses == null || expenses.isEmpty() || user == null)
            return;
        try {
       AuditEvent bulk = AuditEvent.builder()
                    .userId(user.getId())
