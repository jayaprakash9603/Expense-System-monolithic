package com.jaya.budgetservice.service;

import com.jaya.budgetservice.dto.BudgetReport;
import com.jaya.budgetservice.dto.DetailedBudgetReport;
import com.jaya.budgetservice.dto.ExpenseBudgetLinkingEvent;
import com.jaya.budgetservice.exceptions.BudgetNotFoundException;
import com.jaya.budgetservice.models.Budget;
import com.jaya.budgetservice.repository.BudgetRepository;
import com.jaya.budgetservice.util.ServiceHelper;
...
public class BudgetServiceImpl implements BudgetService {
    ...
    public List<Budget> getAllBudgetForUser(Integer userId) {
        ...
        Map<Integer, Expense> expenseMap = Collections.emptyMap();
        if (!allExpenseIds.isEmpty()) {
            try {
                // Single remote call instead of per-expense N+1
                List<Expense> Expenses = expenseService.getExpensesByIds(userId, allExpenseIds);
                expenseMap = Expenses.stream()
        ...
        for (Budget budget : budgets) {
            ...
            if (expenseIds != null && !expenseIds.isEmpty()) {
                for (Integer expId : expenseIds) {
                    Expense dto = expenseMap.get(expId);
    ...
    public DetailedBudgetReport calculateDetailedBudgetReport(Integer userId, Integer budgetId) throws Exception {
        ...
        List<Expense> allExpenses = expenseService.findByUserIdAndDateBetweenAndIncludeInBudgetTrue(
                budget.getStartDate(),
                budget.getEndDate(),
                userId);
        ...
        double totalSpent = expenses.stream()
                .mapToDouble(e -> (double) e.getExpense().getAmount())
                .sum();
    ...
    private List<DetailedBudgetReport.CategoryExpense> calculateCategoryBreakdown(List<Expense> expenses,
            double totalSpent) {
        ...
        for (Map.Entry<String, List<Expense>> entry : categoryMap.entrySet()) {
            ...
            Map<String, List<Expense>> subcategoryMap = new HashMap<>();
            ...
            for (Map.Entry<String, List<Expense>> subEntry : subcategoryMap.entrySet()) {
    ...
    private List<DetailedBudgetReport.PaymentMethodExpense> calculatePaymentMethodBreakdown(List<Expense> expenses,
            double totalSpent) {
        Map<String, List<Expense>> paymentMap = new HashMap<>();
        ...
        for (Map.Entry<String, List<Expense>> entry : paymentMap.entrySet()) {
    ...
    private List<DetailedBudgetReport.DailySpending> calculateDailySpending(List<Expense> expenses,
            LocalDate startDate,
            LocalDate endDate) {
        Map<LocalDate, List<Expense>> dailyMap = new HashMap<>();
        ...
        for (Map.Entry<LocalDate, List<Expense>> entry : dailyMap.entrySet()) {
    ...
    private List<DetailedBudgetReport.WeeklySpending> calculateWeeklySpending(List<Expense> expenses,
            LocalDate startDate,
            LocalDate endDate) {
        Map<String, List<Expense>> weeklyMap = new HashMap<>();
        ...
        for (Map.Entry<String, List<Expense>> entry : weeklyMap.entrySet()) {
            String week = entry.getKey();
            List<Expense> weekExpenses = entry.getValue();
    ...
    private List<DetailedBudgetReport.ExpenseTransaction> mapToExpenseTransactions(List<Expense> expenses) {
    ...
    private List<DetailedBudgetReport.ComparisonData> calculateComparisonData(
        ...
        try {
            List<Expense> previousExpenses = expenseService.findByUserIdAndDateBetweenAndIncludeInBudgetTrue(
    ...
    private List<DetailedBudgetReport.HourlySpending> calculateHourlySpending(List<Expense> expenses) {
        ...
        for (Expense expense : expenses) {
            double amount = (double) expense.getExpense().getAmount();
    ...
    public Map<String, Object> getSingleBudgetDetailedReport(Integer userId, Integer budgetId, LocalDate fromDate,
        ...
        List<Expense> filteredExpenses = getFilteredExpenses(userId, budgetId, effectiveDateRange, flowType);
    ...
    private DateRange calculateWeekRange(LocalDate baseDate, int offset) {
        LocalDate weekStart = baseDate.plusWeeks(offset).with(DayOfWeek.MONDAY);
     ...
     */
    private List<Expense> getFilteredExpenses(Integer userId, Integer budgetId,
                                              DateRange dateRange, String flowType) throws Exception {

        List<Expense> expenses = getExpensesForUserByBudgetId(userId, budgetId);
     ...
     */
    private List<Expense> filterExpensesByDateRange(List<Expense> expenses, DateRange dateRange) {
     ...
     */
    private List<Expense> filterExpensesByFlowType(List<Expense> expenses, String flowType) {
     ...
     */
    private ExpenseAggregationData aggregateExpenseData(List<Expense> expenses) {
        ExpenseAggregationData data = new ExpenseAggregationData();

        for (Expense dto : expenses) {
     ...
     */
    private void processExpenseForAggregation(Expense dto, ExpenseAggregationData data) {
        String expenseName = dto.getExpense().getExpenseName();
        String paymentMethod = dto.getExpense().getPaymentMethod();
        String category = getCategoryName(dto);
        double amount = (double) dto.getExpense().getAmount();
     ...
     */
    private void updateExpenseGroups(ExpenseAggregationData data, Expense dto,
     ...
     */
    @SuppressWarnings("unchecked")
    private void addExpenseDetailsToGroup(Map<String, Object> group, Expense dto) {
     ...
     */
    private Map<String, Object> buildExpenseDetailsMap(Expense dto) {
     ...
     */
    private Map<String, Object> buildExpenseInnerDetails(Expense dto) {
        Map<String, Object> details = new LinkedHashMap<>();
        details.put("amount", round2((Double) dto.getExpense().getAmount()));
        details.put("comments", dto.getExpense().getComments());
        details.put("netAmount", round2((Double) dto.getExpense().getNetAmount()));
        ...
        details.put("creditDue", round2((Double) dto.getExpense().getCreditDue()));
        return details;
    }
     ...
     */
    private String getCategoryName(Expense dto) {
